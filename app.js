...
const favoriteButton = document.querySelector("#favorite-entry-button");
  ...
  if (isFavorite(entryId)) {
    favoriteButton.textContent = "★ Saved Entry";
    favoriteButton.classList.add("saved");
  } else {
    favoriteButton.textContent = "☆ Save Entry";
...
function renderRelatedEntries(currentEntry) {
  ...
  if (relatedEntries.length === 0) {
    ...
    emptyCard.innerHTML = "<h3>No related entries yet</h3><p>As you add more Pokémon, animals, hazards, tools, and plants, related entries will appear here automatically.</p>";
...
function openTagResults(tag) {
  ...
  if (matchingEntries.length === 0) {
    const emptyCard = document.createElement("article");
    emptyCard.className = "entry-card";
    emptyCard.innerHTML = "<h3>No entries found</h3><p>This tag is ready for future Pokémon, animals, and field entries.</p>";
...
async function openEntryDetail(entry) {
  ...
  setText("#detail-image-path", "Image Path", entry.image || "Not added yet");
...
fetch("./data/entries-index.json?v=20260516-235007")
